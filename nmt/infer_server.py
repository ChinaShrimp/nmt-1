...
def run_main(flags, default_hparams):
  """Run main."""
  # Job
  jobid = flags.jobid
  ...
  hparams = create_or_load_hparams(
      flags.out_dir, default_hparams, flags.hparams_path, save_hparams=(jobid == 0))

  print(hparams)
  model, sess = inference.create_inference(flags.ckpt, hparams, num_workers, jobid)

  return hparams, model, sess


def Init(unused_argv):
  default_hparams = create_hparams(FLAGS)

  hparams, model, sess = run_main(FLAGS, default_hparams)

  return hparams, model, sess


class InferenceTCPHandler(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        ...
        translation = inference.translate(self.server.hparams, self.server.model,
            self.server.sess, text)
...
class InferenceServer(socketserver.ThreadingMixIn, socketserver.TCPServer):
    ...
    def __init__(self, server_address, RequestHandlerClass, unused_argv):
        socketserver.TCPServer.__init__(self, server_address, RequestHandlerClass)

        # Initialize GNMT for further inferences
        self.hparams, self.model, self.sess = Init(unused_argv)
...
def server(unused_argv):
  print("hello to GNMT inference server!")

  server = InferenceServer((HOST, PORT), InferenceTCPHandler, unused_argv)
...
if __name__ == "__main__":
  ...
  tf.app.run(main=server, argv=[sys.argv[0]] + unparsed)
